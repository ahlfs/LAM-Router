...
func TestLoadConfigDefaults(t *testing.T) {
	...
	if cfg.Port != 9898 { // Default port
		t.Errorf("expected default port 9898, got %d", cfg.Port)
	}
...
func TestLoadConfigInvalidPort(t *testing.T) {
	...
	cfg := LoadConfig()
	if cfg.Port != 9898 {
		t.Errorf("expected fallback port 9898 for invalid port, got %d", cfg.Port)
	}

	t.Setenv("PORT", "-5")
	cfg2 := LoadConfig()
	if cfg2.Port != 9898 {
		t.Errorf("expected fallback port 9898 for negative port, got %d", cfg2.Port)
	}
}
