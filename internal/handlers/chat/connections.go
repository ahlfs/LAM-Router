...
func (h *ChatHandler) getBestConnection(provider string, connectionID string, excludeIDs []string, model string) (*models.ProviderConnection, *ConnectionData, error) {
	...
	if connectionID != "" {
		...
		if len(connections) == 0 {
			// Fallback: check providers table directly (handles imported or UI-configured providers)
			var provID, provName, provBaseURL, provAPIKey, provAccessToken, provRefreshToken, provHeaders, provSpecificData sql.NullString
			row := h.Repo.RawDB().QueryRow(`SELECT id, name, base_url, api_key, access_token, refresh_token, custom_headers, provider_specific_data 
				FROM providers WHERE (LOWER(provider_id) = LOWER(?) OR LOWER(id) = LOWER(?)) AND enabled = 1 LIMIT 1`, provider, provider)
			if err := row.Scan(&provID, &provName, &provBaseURL, &provAPIKey, &provAccessToken, &provRefreshToken, &provHeaders, &provSpecificData); err == nil {
				nameStr := provName.String
				connDataMap := make(map[string]any)
				if provAPIKey.Valid && provAPIKey.String != "" {
					connDataMap["apiKey"] = provAPIKey.String
				}
				if provAccessToken.Valid && provAccessToken.String != "" {
					connDataMap["accessToken"] = provAccessToken.String
				}
				if provRefreshToken.Valid && provRefreshToken.String != "" {
					connDataMap["refreshToken"] = provRefreshToken.String
				}
				if provBaseURL.Valid && provBaseURL.String != "" {
					connDataMap["baseUrl"] = provBaseURL.String
				}
				if provSpecificData.Valid && provSpecificData.String != "" {
					var psd map[string]any
					if err := json.Unmarshal([]byte(provSpecificData.String), &psd); err == nil {
						for k, v := range psd {
							connDataMap[k] = v
						}
					}
				}
				dataBytes, _ := json.Marshal(connDataMap)
				dynConn := &models.ProviderConnection{
					ID:       provID.String,
					Provider: provider,
					Name:     &nameStr,
					IsActive: 1,
				...
				connections = append(connections, dynConn)
			}
		}
...
func (h *ChatHandler) getProviderConfig(provider string, connData *ConnectionData) (*providers.ProviderConfig, error) {
	...
	} else if cfg, ok := providers.GetKnownProviderConfig(provider); ok {
