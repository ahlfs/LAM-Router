...
func (h *ChatHandler) forwardGeminiNativeRequest(
	...
	if projectID == "" {
		if provider == "antigravity" {
			// Fallback: Use Google Antigravity standard default internal project ID if discovery endpoint hasn't populated one yet
			projectID = "antigravity-default"
		} else {
...
func (h *ChatHandler) forceRefreshOAuthToken(connectionID string) (string, string, error) {
	...
	if oauthData != nil && oauthData.RefreshToken != "" {
		refreshToken = oauthData.RefreshToken
	} else if key, ok := connMap["apiKey"].(string); ok && key != "" {
		// Fallback for cookie / session key stored as apiKey (e.g. Framer)
		refreshToken = key
	}

	if refreshToken == "" {
		return "", "", fmt.Errorf("no refresh token or session key available")
	}
	...
	if refresher := oauth.Get(provider); refresher != nil {
		...
		if err == nil && result != nil {
			...
			existing["apiKey"] = result.AccessToken
			if result.ProjectID != "" {
				existing["projectId"] = result.ProjectID
			}
			...
			if err != nil {
				...
				db.Exec("UPDATE providerConnections SET data = ?, updatedAt = ? WHERE id = ?",
					string(mergedJSON), time.Now().UTC().Format(time.RFC3339), connectionID)
				db.Exec("UPDATE providers SET api_key = ?, access_token = ? WHERE id = ?",
					result.AccessToken, result.AccessToken, connectionID)
			}
			return result.AccessToken, result.ProjectID, nil
		}
	}
