...
import (
	...
	"9router/proxy/internal/webdist"
)
...
func runServer(cCtx *cli.Context) error {
	...
	// Register SRouter v1 REST API for UI dashboard
	srouterapi.RegisterV1Routes(r, repo, ts)

	// Mount standard 9router AI API routes under /v1 for OpenAI/Anthropic/Media
	r.Route("/v1", func(v1 chi.Router) {
		handlers.SetupV1ProxyRoutes(v1, repo, ts)
	})

	handlers.SetupServerRouter(r, repo, ts)
	webdist.RegisterWebUI(r)
